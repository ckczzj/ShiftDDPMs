...
class ShiftTrainer(BaseTrainer):
    ...
    def _build_model(self):
        ...
        self.ema_shift_predictor = DistributedDataParallel(copy.deepcopy(shift_predictor).cuda(), device_ids=[self.device])
    ...
    def _build_optimizer(self):
        ...
        self.optimizer = Adam(
            [
                ...
                {"params": self.shift_predictor.parameters()},
            ],
        ...
        while True:
            ...
            with torch.cuda.amp.autocast(enabled=self.enable_amp):
                ...
                output = self.gaussian_diffusion.shift_train_one_batch(
                    ...
                    condition=move_to_cuda(batch["net_input"]["condition"]),
                )
    ...
    def accumulate(self, decay):
        ...
        shift_predictor_parameter = dict(self.shift_predictor.named_parameters())
    ...
    def save(self, path):
        data = {
            ...
            'shift_predictor': self.shift_predictor.module.state_dict(),
            'ema_shift_predictor': self.ema_shift_predictor.module.state_dict(),
    ...
    def load(self, path):
        ...
        self.shift_predictor.module.load_state_dict(data['shift_predictor'])
        self.ema_shift_predictor.module.load_state_dict(data['ema_shift_predictor'])
